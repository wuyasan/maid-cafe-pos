...
def get_staff_tables_overview(db: Session = Depends(get_db)):
    ...
    for session_table, table in rows:
        open_bill = get_open_or_paying_bill_for_session_table(db, session_table.id)
    ...
    tables.append(
        SessionTableSummary(
            ...
            open_bill_total=open_bill.total if open_bill else Decimal("0.00"),
        )
    )
...
def start_checkout(table_code: str, db: Session = Depends(get_db)):
    _current_session, session_table, _table = get_current_session_table_by_code(db, table_code)

    bill = get_open_or_paying_bill_for_session_table(db, session_table.id)
    if not bill:
        raise HTTPException(status_code=404, detail="No open bill found for this table.")
...
def mark_paid(table_code: str, db: Session = Depends(get_db)):
    _current_session, session_table, _table = get_current_session_table_by_code(db, table_code)

    bill = get_open_or_paying_bill_for_session_table(db, session_table.id)
    if not bill:
        raise HTTPException(status_code=404, detail="No open or paying bill found for this table.")

    bill.status = BillStatus.paid
    bill.closed_at = datetime.utcnow()
    session_table.status = SessionTableStatus.paid
